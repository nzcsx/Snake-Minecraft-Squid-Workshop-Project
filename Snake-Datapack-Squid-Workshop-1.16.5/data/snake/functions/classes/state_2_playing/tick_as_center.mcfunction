...
execute as @a if score @s snake_puid = @e[tag=this_center,nbt={Age:1},limit=1] snake_puid run function snake:classes/state_2_playing/input/as_player
...
#   Eat food or
#   Move forward
execute as @e[tag=snake_head] if score @s snake_cuid = @e[tag=this_center,nbt={Age:1},limit=1] snake_cuid at @s positioned ^ ^ ^1 run function snake:classes/state_2_playing/eat_or_move/as_head
